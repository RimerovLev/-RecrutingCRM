...
def get_or_create_user(telegram_id: int, username: str | None, full_name: str | None) -> str:
    """Возвращает recruiter_id для данного telegram_id, создаёт если нет."""
...
def get_recruiter_id(telegram_id: int) -> str | None:
    """Возвращает recruiter_id или None если пользователь не зарегистрирован."""
    res = sb.from_("telegram_users").select("recruiter_id").eq("telegram_id", telegram_id).execute()
...
async def require_user(update: Update) -> str | None:
    """Возвращает recruiter_id или отправляет сообщение с просьбой /start."""
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    rid = get_or_create_user(
        telegram_id=user.id,
        username=user.username,
        full_name=user.full_name,
    )
    is_new = True
    # Проверяем был ли уже зарегистрирован (по тому что get_recruiter_id уже был)
    ...
    if res.data:
        ...
        is_new = delta.total_seconds() < 10  # только что создан

    if is_new:
        text = (
            f"🎉 <b>Добро пожаловать в Recruit CRM Bot!</b>\n\n"
            f"Твой аккаунт создан. Теперь ты можешь управлять кандидатами и вакансиями прямо из Telegram.\n\n"
        )
    else:
