...
export default function TelegramLinkModal() {
  ...
  const generate = async () => {
    ...
    if (error) { addToast(t('telegram.error') + ' ' + error.message, 'err'); return; }
    setCode(c);
    setStep('generated');
  ...
  const copyCmd = () => {
    navigator.clipboard.writeText(`/link ${code}`).then(() => addToast(t('telegram.copied')));
  };

  return (
    <Modal open={open} onClose={handleClose} title={t('telegram.title')}>
      {step === 'idle' ? (
        <div className="text-center space-y-4">
          <p className="text-5xl">🤖</p>
          <p className="text-slate-600 text-sm">
            {t('telegram.desc')} <code className="bg-slate-100 px-1 py-0.5 rounded text-xs">/link КОД</code>
          </p>
          <button onClick={generate} className="btn-primary w-full justify-center py-2.5">
            {t('telegram.generateBtn')}
          </button>
        </div>
      ) : (
        <div className="space-y-4">
          <div className="text-center bg-indigo-50 rounded-xl p-5">
            ...
            <p className="text-xs text-slate-500">{t('telegram.botCmd')}</p>
            <code className="text-sm font-bold text-slate-800">/link {code}</code>
          </div>
          <div className="flex gap-2">
            <button onClick={copyCmd} className="btn-primary flex-1 justify-center">{t('telegram.copyBtn')}</button>
            <button onClick={generate} className="btn-secondary px-4">↺</button>
          </div>
          <p className="text-xs text-slate-400 text-center">
            {timer && timer !== 'expired'
              ? `${t('telegram.expires')} ${timer}`
              : t('telegram.expired')}
          </p>
        </div>
      )}
